...
// Create a DatabaseStorage class that implements IStorage using PostgreSQL and Drizzle ORM
import { db } from "./db";
...
export class DatabaseStorage implements IStorage {
  ...
  async getContractWithMeta(id: number): Promise<ContractWithMeta | undefined> {
    ...
    const [creator] = contract.createdBy 
      ? await db.select({ username: users.username, fullName: users.fullName })
          .from(users)
          .where(eq(users.id, contract.createdBy))
      : [null];

    // Get approvals if any
    const approvalsList = await db.select().from(contractApprovals)
      .where(eq(contractApprovals.contractId, id));
    
    // Enrich approvals with approver data
  ...
  async listContractsByUser(userId: number): Promise<Contract[]> {
    return await db.select().from(contracts).where(eq(contracts.createdBy, userId));
  }
  ...
  async searchContracts(query: string): Promise<Contract[]> {
    // Case-insensitive search in name, parties, and description
    return await db.select().from(contracts).where(
      or(
        eq(contracts.name, query),
        eq(contracts.parties, query),
        eq(contracts.description, query)
      )
    );
  }
  ...
  async listPendingApprovalsByUser(approverId: number): Promise<ContractWithApproval[]> {
    // Get all approvals for the user with status = "Pending"
    ...
    // Get the corresponding contracts
    const result: ContractWithApproval[] = [];
    
    for (const approval of pendingApprovals) {
      const contractWithMeta = await this.getContractWithMeta(approval.contractId);
      
      if (contractWithMeta) {
        result.push(contractWithMeta as ContractWithApproval);
      }
    }
...
// Initialize PostgreSQL database and create a DatabaseStorage instance
